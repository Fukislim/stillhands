public enum KeyNames {
    ...
    public static func name(for keyCode: UInt16) -> String? { table[keyCode] }

    public static func code(for name: String) -> UInt16? {
        table.first { $0.value == name }?.key
    }

    public static let randomPool: [UInt16] = table
        .filter { $0.value.count == 1 && ($0.value.first!.isLetter || $0.value.first!.isNumber) }
        .map(\.key)
        .sorted()
}
